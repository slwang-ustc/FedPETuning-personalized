...
def setup_logger(config):
    ...
    log_path = os.path.join(config.T.output_dir, config.F.fl_algorithm.lower(), 'server_client_log')
    now = time.strftime("%Y-%m-%d-%H_%M", time.localtime(time.time()))
    if config.F.rank == 0:
        log_path = os.path.join(log_path, now + '_server.log')
    elif config.F.rank > 0:
        log_path = os.path.join(log_path, now + '_client.log')
    else:
        log_path = os.path.join(log_path, now + '_centralized.log')

    logger.remove()

    logger.add(
        log_path, format=formatter,
        # colorize=True, 
        enqueue=True,
        encoding="utf-8"
    )
