...
import sys
sys.path.append('/data/slwang/FedPETuning/code/FedPETuning')
...
def get_partition_data(
    ...
    clients_partition_data = GlueDataPartition(
        ...
        proportions_client_list=proportions_client_list, dataset_type=dataset_type
    )
    ...
    if dataset_type == 'train':
        proportions_client_list_return = copy.deepcopy(clients_partition_data.proportions_client_list)
        return partition_data, proportions_client_list_return
    else:
...
def convert_glue_to_device_pkl(args):
    ...
    if f"clients={args.clients_num}_alpha={args.alpha}" in partition_data and not args.overwrite:
        ...
        valid_partition_data = get_partition_data(
            ...
            proportions_client_list=proportions_client_list, dataset_type='valid'
        )
        test_partition_data = get_partition_data(
